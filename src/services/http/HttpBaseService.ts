...
/**
 * High-level configuration contract applied to client transaction calls.
 */
export interface HttpTransactionOptions {
  /**
   * Connects mid-flight operations to outer abort signals or cancellation hooks.
   */
  readonly signal?: AbortSignal;
  /**
   * Custom request execution timeout duration in milliseconds overriding default network limits.
   */
  readonly timeout?: number;
}

/**
 * Extended low-level request configuration options supporting custom network operation timeouts and standard fetch options.
 */
export type HttpRequestOptions = RequestInit & HttpTransactionOptions;
...
export abstract class HttpBaseService {
  ...
  protected async request<R>(
    ...
    const effectiveTimeout = options?.timeout ?? NETWORK_CONFIG.TIMEOUT;
  ...
  private createTimeoutSignal(
    ...
    const timeout = timeoutMs ?? NETWORK_CONFIG.TIMEOUT;
