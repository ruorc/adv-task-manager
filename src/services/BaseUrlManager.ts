...
export class BaseUrlManager {
  ...
  public resolve(pathSegment?: string, searchParams?: URLSearchParams): URL {
    // Remove leading slashes to avoid resetting the pathname of baseUrl
    const sanitizedSegment = pathSegment?.replace(/^\/+/, '');

    const targetUrl = sanitizedSegment
      ? new URL(sanitizedSegment, this.baseUrl)
